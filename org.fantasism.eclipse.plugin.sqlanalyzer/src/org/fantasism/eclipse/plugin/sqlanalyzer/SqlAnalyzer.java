...
public class SqlAnalyzer {
    ...
    public void analyze(SqlAnalyzerContext context, String sql) {

        context.getListener().beginAnalyzeing(context.getQueryName(), sql);

        SQLQueryParserManager manager = SQLQueryParserManagerProvider.getInstance().getParserManager(null, null);
        try {

            String postSql = context.getListener().processSql(context.getQueryName(), sql);

            SQLQueryParseResult result = manager.parseQuery(postSql);

            context.getStatementAnalyzer().analyze(context.getRootQuery(), result.getQueryStatement());
            ...
            context.getListener().endAnalyzeing(context.getQueryName(), sql, context.getRootQuery());
        }
...
//
//            SQLQueryParseResult result = manager.parseQuery(selectInsert.toString());
//
//            Query query = new Query(null);
//            SqlAnalyzerManager.getInstance().getStatementAnalyzer().analyze(query, result.getQueryStatement());
//
//
